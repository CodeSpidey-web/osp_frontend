export default function RelatedProducts() {
    return (
        <>
            <div className="rbt-component-area rbt-section-gap rbt-bg-color-gray-light">
                <div className="container">
                    <div className="rbt-fshape-box-outline-style rbt-fshape-box-outline-style-primary-stroke rbt-fshape-box-outline-style-extend-width rbt-product-fshape-box-outline-style">
                        <div className="row rbt-section-gap2Top pt_sm--100 pt_md--80">
                            <div className="col-lg-12">
                                <div className="rbt-component-section-title rbt-bg-color-white">
                                    <h2 className="rbt-title h4"><span className="rbt-bold--text">Compare similar items</span></h2>
                                    <span className="rbt-fshape-right-portion">
                                        <svg fill="none" height="50" viewBox="0 0 52 50" width="52" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M51.5337 49.984C-64.8544 49.9977 116.427 49.9764 0.0390625 49.9901C0.0390625 31.262 0.0390625 20.7619 0.0390625 2.03378C11.2391 1.63419 16.5034 4.56468 19.5034 10.5602L30.0034 38.5311C34.0374 47.934 45.4209 49.4481 51.5337 49.984Z" fill="var(--color-white)"></path>
                                            <path clipRule="evenodd" d="M13.246 1.97519C16.582 3.50685 18.8114 5.90944 20.3979 9.07997L20.4213 9.12681L30.9315 37.1248C33.053 42.053 36.807 44.7979 40.7367 46.3047C44.6934 47.8219 48.798 48.068 51.4731 47.987C51.4731 47.987 51.51 49.2041 51.5337 49.984C48.7087 50.0695 44.3134 49.8162 40.02 48.17C35.7052 46.5155 31.4643 43.4388 29.0842 37.891L29.0751 37.8698C29.0751 37.8698 19.997 12.7279 18.5857 9.92689C17.1743 7.12591 15.2591 5.09828 12.4108 3.79055C8.49554 1.49902 0.0390625 2.03378 0.0390625 2.03378C0.0390625 20.7619 0.0390625 31.262 0.0390625 49.9901L0.0408325 0.0348727C5.70805 -0.16568 9.9493 0.461575 13.246 1.97519Z" fill="var(--color-primary)" fillRule="evenodd"></path>
                                        </svg>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div className="rbt-component-area rbt-fshape-box rbt-bg-color-white">
                            <div className="row">
                                <div className="col-12 rbt-scrollable-content">
                                    {/* Start Compare Table */}
                                    <table className="rbt-compare-table">
                                        <tbody>
                                            <tr>
                                                <td></td>
                                                <td>
                                                    <div className="rbt-input-field-grp">
                                                        <input className="rbt-input-field" placeholder="Search and Select Product" type="text" />
                                                        <button className="rbt-search-btn"><i className="fa-sharp fa-solid fa-magnifying-glass"></i></button>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-input-field-grp">
                                                        <input className="rbt-input-field" placeholder="Search and Select Product" type="text" />
                                                        <button className="rbt-search-btn"><i className="fa-sharp fa-solid fa-magnifying-glass"></i></button>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-input-field-grp">
                                                        <input className="rbt-input-field" placeholder="Search and Select Product" type="text" />
                                                        <button className="rbt-search-btn"><i className="fa-sharp fa-solid fa-magnifying-glass"></i></button>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-input-field-grp">
                                                        <input className="rbt-input-field" placeholder="Search and Select Product" type="text" />
                                                        <button className="rbt-search-btn"><i className="fa-sharp fa-solid fa-magnifying-glass"></i></button>
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr className="rbt-compare-prd-table-head">
                                                <td className="rbt-compare-table-title">
                                                    <div className="rbt-compare-values">
                                                        <span>
                                                            <svg fill="none" height="56" viewBox="0 0 56 56" width="56" xmlns="http://www.w3.org/2000/svg">
                                                                <path d="M28 7.5C25.0767 7.5 22.6975 9.88262 22.6975 12.8112C22.6936 13.7244 22.9272 14.6229 23.3755 15.4184C23.8237 16.214 24.4712 16.8793 25.2543 17.349C26.0823 17.8506 27.0319 18.1157 28 18.1157C28.9681 18.1157 29.9177 17.8506 30.7458 17.349C31.5289 16.8794 32.1764 16.2141 32.6247 15.4185C33.073 14.6229 33.3066 13.7244 33.3025 12.8112C33.3025 9.88262 30.9234 7.5 28 7.5ZM35.1392 42.6269H31.1667L31.1675 42.6225V19.1112C30.1963 19.61 29.1025 19.8638 28 19.8638C26.8975 19.8638 25.8038 19.61 24.8325 19.1112V42.6225L24.8334 42.6269H20.8609C19.8457 42.628 18.8723 43.0318 18.1544 43.7496C17.4364 44.4674 17.0324 45.4406 17.031 46.4559V47.6258C17.031 47.8578 17.1232 48.0804 17.2873 48.2445C17.4514 48.4086 17.674 48.5008 17.906 48.5008H38.094C38.3261 48.5008 38.5487 48.4086 38.7127 48.2445C38.8768 48.0804 38.969 47.8578 38.969 47.6258V46.4559C38.9676 45.4406 38.5637 44.4674 37.8457 43.7496C37.1277 43.0318 36.1544 42.628 35.1392 42.6269ZM12.2439 33.8524C15.9495 33.8524 18.9657 30.8371 18.9657 27.1306C18.9657 26.8629 18.9989 25.8575 18.8922 25.6134L18.8904 25.6055L13.5914 13.6871H21C20.9292 13.1059 20.9292 12.5183 21 11.9371H12.2395C12.0724 11.9416 11.9097 11.9922 11.7695 12.0833C11.6293 12.1744 11.517 12.3025 11.445 12.4534L5.60003 25.6037L5.59915 25.6064C5.4889 25.854 5.52128 26.8506 5.52128 27.1297C5.52215 30.8363 8.5374 33.8524 12.2439 33.8524ZM12.243 14.9611L16.744 25.0849H7.73853L12.243 14.9611ZM50.477 25.9529C50.4762 25.9161 50.4604 25.8803 50.4552 25.8435C50.4438 25.7612 50.4333 25.6799 50.4009 25.6064L50.4 25.6037L44.555 12.4525C44.4837 12.3013 44.3716 12.1729 44.2315 12.0818C44.0913 11.9906 43.9286 11.9402 43.7614 11.9362H35C35.0709 12.5174 35.0709 13.1051 35 13.6862H42.4086L37.1097 25.6046L37.1079 25.6125C37.086 25.6633 37.0834 25.7219 37.0712 25.777C37.0589 25.8356 37.0362 25.8934 37.0362 25.952L37.0344 25.959V27.1289C37.0344 30.8354 40.0497 33.8506 43.757 33.8506C47.4627 33.8506 50.4788 30.8354 50.4788 27.1289V25.959L50.477 25.9529ZM39.256 25.0849L43.757 14.9611L48.2615 25.0849H39.256Z" fill="#E6E6E6"></path>
                                                            </svg>
                                                        </span>
                                                        <p className="rbt-compare-table-text">Find and select products to see the
                                                            differences
                                                            and similarities between them</p>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-item-wrapper">
                                                        <button className="rbt-product-remove-btn">
                                                            <i className="fa-sharp fa-solid fa-xmark"></i>
                                                        </button>
                                                        <a className="rbt-product-item-img rbt-bg-color-brand-100 rbt-scroll-trigger fade_in animation-order-1" href="product-single-default.html">
                                                            <img alt="Product Image" src="/assets/images/compare-products/compare-prd-img-elc-1.webp" />
                                                        </a>
                                                        <div className="rbt-compare-values">
                                                            <a className="rbt-product-item-category" href="shop-by-categories.html">
                                                                Apple AirPods
                                                            </a>
                                                            <p className="rbt-product-item-title h6">
                                                                <a href="product-single-default.html">Apple AirPods (3rd
                                                                    Generation) with MagSafe</a>
                                                            </p>
                                                            <a className="rbt-btn rbt-btn-sm has-left-icon" href="#">
                                                                <i className="fa-regular fa-cart-shopping"></i>
                                                                Add To Cart
                                                            </a>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-item-wrapper">
                                                        <button className="rbt-product-remove-btn">
                                                            <i className="fa-sharp fa-solid fa-xmark"></i>
                                                        </button>
                                                        <a className="rbt-product-item-img rbt-bg-color-brand-100 rbt-scroll-trigger fade_in animation-order-2" href="product-single-default.html">
                                                            <img alt="Product Image" src="/assets/images/compare-products/compare-prd-img-elc-2.webp" />
                                                        </a>
                                                        <div className="rbt-compare-values">
                                                            <a className="rbt-product-item-category" href="shop-by-categories.html">
                                                                Galaxy Buds
                                                            </a>
                                                            <p className="rbt-product-item-title h6">
                                                                <a href="product-single-default.html">Samsung Galaxy Buds 2 Pro
                                                                    Wireless Earbuds</a>
                                                            </p>
                                                            <a className="rbt-btn rbt-btn-sm has-left-icon" href="#">
                                                                <i className="fa-regular fa-cart-shopping"></i>
                                                                Add To Cart
                                                            </a>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-item-wrapper">
                                                        <button className="rbt-product-remove-btn">
                                                            <i className="fa-sharp fa-solid fa-xmark"></i>
                                                        </button>
                                                        <a className="rbt-product-item-img rbt-bg-color-brand-100 rbt-scroll-trigger fade_in animation-order-3" href="product-single-default.html">
                                                            <img alt="Product Image" src="/assets/images/compare-products/compare-prd-img-elc-3.webp" />
                                                        </a>
                                                        <div className="rbt-compare-values">
                                                            <a className="rbt-product-item-category" href="shop-by-categories.html">
                                                                Sony Earbuds
                                                            </a>
                                                            <p className="rbt-product-item-title h6">
                                                                <a href="product-single-default.html">Sony WF-1000XM5
                                                                    Noise-Canceling Earbuds</a>
                                                            </p>
                                                            <a className="rbt-btn rbt-btn-sm has-left-icon" href="#">
                                                                <i className="fa-regular fa-cart-shopping"></i>
                                                                Add To Cart
                                                            </a>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-item-wrapper">
                                                        <button className="rbt-product-remove-btn">
                                                            <i className="fa-sharp fa-solid fa-xmark"></i>
                                                        </button>
                                                        <a className="rbt-product-item-img rbt-bg-color-brand-100 rbt-scroll-trigger fade_in animation-order-4" href="product-single-default.html">
                                                            <img alt="Product Image" src="/assets/images/compare-products/compare-prd-img-elc-4.webp" />
                                                        </a>
                                                        <div className="rbt-compare-values">
                                                            <a className="rbt-product-item-category" href="shop-by-categories.html">
                                                                Apple AirPods
                                                            </a>
                                                            <p className="rbt-product-item-title h6">
                                                                <a href="product-single-default.html">Apple AirPods Max Over-Ear
                                                                    Headphones</a>
                                                            </p>
                                                            <a className="rbt-btn rbt-btn-sm has-left-icon" href="#">
                                                                <i className="fa-regular fa-cart-shopping"></i>
                                                                Add To Cart
                                                            </a>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className="rbt-product-feature-name">Customer Rating</td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        <div className="rbt-card-rating">
                                                            <ul className="rbt-rating-icon-list">
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                            </ul>
                                                            <p className="rating-digit">(46)</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        <div className="rbt-card-rating">
                                                            <ul className="rbt-rating-icon-list">
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                            </ul>
                                                            <p className="rating-digit">(46)</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        <div className="rbt-card-rating">
                                                            <ul className="rbt-rating-icon-list">
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                            </ul>
                                                            <p className="rating-digit">(46)</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        <div className="rbt-card-rating">
                                                            <ul className="rbt-rating-icon-list">
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                                <li><i className="fa-solid fa-star rbt-rated-icon"></i></li>
                                                            </ul>
                                                            <p className="rating-digit">(46)</p>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className="rbt-product-feature-name">Price</td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        <span className="rbt-product-price">₹2,999</span>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        <span className="rbt-product-price">₹2,999</span>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        <span className="rbt-product-price">₹2,999</span>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        <span className="rbt-product-price">₹2,999</span>
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className="rbt-product-feature-name">Sold By</td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        A4C Online
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        World Electronics
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        Transcom Digital
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        Esquare Electric
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className="rbt-product-feature-name">Color</td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        Black
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        Green
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        Black
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        Gray
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className="rbt-product-feature-name">Fit Type</td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        In-Ear
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        In-Ear
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        In-Ear
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        In-Ear
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className="rbt-product-feature-name">Item Dimensions</td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        0.67 x 0.79 x 1.65 inches
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        -----
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        1.73 x 3.94 x 6.3 inches
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        6.5 x 4 x 2 inches
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className="rbt-product-feature-name">Special Features</td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        wireless, voice assistant, Bluetooth, noise cancellation, adaptive,
                                                        wireless
                                                        charging
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        Lightweight, Microphone Included, Fast Charging, Noise Cancellation
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        wireless, voice assistant, Bluetooth, noise cancellation, adaptive,
                                                        wireless
                                                        charging
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="rbt-compare-values">
                                                        wireless, voice assistant, Bluetooth, noise cancellation, adaptive,
                                                        wireless
                                                        charging
                                                    </div>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                    {/* End Compare Table */}
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </>
    );
}
